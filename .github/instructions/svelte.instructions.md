...
- In `.svelte` component scripts, do not use generic arrow functions (for example, `const request = async <T>(url: string) => ...`). Use a generic function declaration (`async function request<T>(url: string) { ... }`) or move the helper to a `.ts` module. Svelte 5.56.9 can compile the generic-arrow form without its parameters, causing runtime `ReferenceError`s.
