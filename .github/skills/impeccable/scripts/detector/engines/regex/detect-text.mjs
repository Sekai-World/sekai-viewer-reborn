...
const REGEX_ANALYZERS = [
  ...
  (content, filePath) => {
    ...
    if (count < 3) return [];
    return [
      finding("aphoristic-cadence", filePath, `${count} aphoristic constructions: "${firstSample}"`)
    ];
  },
      ...
      if (pxVals.length >= 3 && pxVals[2] > 4) {
        ...
        return [
          finding(
            "dark-glow",
            filePath,
            `Colored glow (rgb(${r},${g},${b})) on dark page`,
            lines.length
          )
        ];
      }
...
function detectText(content, filePath, options = {}) {
  ...
  for (const f of findings) {
    const isDupe = deduped.some(
      (d) =>
        d.antipattern === f.antipattern && d.snippet === f.snippet && Math.abs(d.line - f.line) <= 2
    );
