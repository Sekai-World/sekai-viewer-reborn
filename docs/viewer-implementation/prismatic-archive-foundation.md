...
- The homepage current event is a data-driven Archive Banner: use the selected region's existing streamed current-event payload while preserving its event-detail link, unit metadata, countdown, and asset retry/fallback behavior. For intrinsically small event artwork, use a compact contained preview rather than a stretched hero: at `lg` widths, a modest fixed media column sits beside the content-led details within the shell's shared `max-w-384` track; mobile remains naturally stacked. Loading, empty, and error states reserve that same media/details geometry to limit shift.
