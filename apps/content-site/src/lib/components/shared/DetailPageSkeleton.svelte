<script lang="ts">
  ...
  const loadingLabel = $derived(
    tCommon(String(page.data.uiLocale ?? ""), "detailLoading", "Loading details...")
  );
</script>
...
<div role="status" aria-busy="true" aria-label={loadingLabel}>
  <div
    aria-hidden="true"
    class:md:grid-cols-[minmax(0,min(33%,400px))_minmax(0,1fr)]={!isMusic}
    class:lg:grid-cols-[minmax(0,min(33%,400px))_minmax(0,1fr)]={true}
