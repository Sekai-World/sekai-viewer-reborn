    ...
    {:else}
      <div class="grid grid-cols-4 gap-1.5 md:grid-cols-[repeat(auto-fill,96px)]">
        {#each pickupCards as pickup (pickup.cardId ?? Math.random())}
          {#if pickup.cardId}
            <a href={getCardDetailHref(pickup.cardId)} class="group block">
              <div
              ...
              >
                <CardThumbnail
                  ...
                  rarityCount={pickup.rarityType === "rarity_birthday"
                    ? 1
                    : getRarityValue(pickup.rarityType)}
                  showFrame={true}
                  showIcons={true}
                  loadMode="visible"
                  maxSize={96}
                ...
                {#if pickup.weight !== null && pickup.weight !== undefined}
                  <div
                    class="flex items-center justify-center bg-linear-to-r from-primary/10 to-transparent px-1 py-0.5 text-center text-[0.6rem] font-medium opacity-60"
                  >
                    {pickup.weight}
                  </div>
                {/if}
          ...
          {:else}
            <div class="overflow-hidden rounded-lg bg-base-200/30 ring-1 ring-base-content/5">
              <div class="relative mx-auto aspect-square overflow-hidden rounded-lg">
                <div
                  class="flex size-full items-center justify-center bg-base-200/50 text-xs opacity-50"
                >
                  ?
                </div>
              </div>
