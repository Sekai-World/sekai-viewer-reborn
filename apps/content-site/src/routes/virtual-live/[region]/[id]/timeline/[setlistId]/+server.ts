...
const enrichTimeline = async (
  document: VirtualLiveTimelineDocument,
  baseUrl: string,
  region: SupportedRegion,
  bundle: string
): Promise<VirtualLiveTimelineDocument> => {
  ...
  const ids = [
    ...new Set(
      document.events
        .flatMap((event) => [event.character3dId, event.targetCharacter3dId])
  ...
  try {
    let response;
    try {
      response = await getCharacter3DsByRegionBatch({
        baseUrl,
        path: { region },
        query: { ids: ids.slice(0, 100).join(",") },
    ...
    const byID = new Map<number, Character3DRecord>(
      (Array.isArray(records) ? records : []).flatMap((value) => {
        const record = getObject(value);
        const id = getNumber(record?.id);
        const gameCharacterId = getNumber(record?.gameCharacterId);
        return id !== null && gameCharacterId !== null
          ? [
              [
                id,
                {
                  id,
                  gameCharacterId,
                  unit: getString(record?.unit) ?? undefined,
    ...
    const gameCharacterIds = [
      ...new Set([...byID.values()].map((record) => record.gameCharacterId))
    ];
    const localizedNames = await resolveLocalizedGameCharacterNames(
      baseUrl,
      region,
      gameCharacterIds,
      characterController
    ...
    const enrich = <T extends VirtualLiveTimelineCharacter | VirtualLiveTimelineEvent>(
      value: T
    ): T => {
    ...
    return {
      ...
      events: documentWithVoices.events.map((event) => {
        ...
        const target =
          event.targetCharacter3dId === null ? null : byID.get(event.targetCharacter3dId);
        return {
          ...enriched,
          targetGameCharacterId: target?.gameCharacterId ?? null,
          targetDisplayName: displayNameFor(target)
        };
      })
    };
  } catch {
...
const readLimitedJson = async (response: Response): Promise<unknown> => {
  ...
  if (contentLength !== null && contentLength > MAX_TIMELINE_BYTES)
...
export const GET: RequestHandler = async ({ params, fetch }) => {
  ...
  if (
    !isSupportedVirtualLiveRegion(region) ||
    !isPositiveSafeInteger(virtualLiveId) ||
    !isPositiveSafeInteger(setlistId)
  ) {
  ...
  try {
    ...
    if (cause instanceof DOMException && cause.name === "AbortError")
      throw error(504, "Timeline request timed out");
    throw cause;
  } finally {
  ...
  if (!bundleName || !/^[A-Za-z0-9_-]+$/.test(bundleName))
    throw error(404, "Timeline asset not found");
  const assetPath =
    setlistType === "mc"
