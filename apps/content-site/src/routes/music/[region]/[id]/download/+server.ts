...
const SUPPORTED_DOWNLOAD_FORMATS = new Set(["mp3", "flac", "wav"]);
...
export const GET: RequestHandler = async ({ params, fetch, request, url }) => {
  ...
  if (!musicId) {
    throw error(400, "Missing music id.");
  }
  ...
  if (!SUPPORTED_DOWNLOAD_FORMATS.has(format)) {
    throw error(400, "Unsupported music download format.");
  }

  const vocalId = url.searchParams.get("vocalId")?.trim() ?? "";
  const previewMode = url.searchParams.get("mode") === "short" ? "short" : "long";
  ...
  try {
    ...
    const music = parseMusicDetail(response.data);
    if (!music) {
      throw error(404, "Music not found.");
    }

    // Find the vocal to download
    const targetVocal = vocalId
      ? music.vocals.find((v) => v.id === vocalId)
      : music.vocals.find((v) => v.assetBundleName && v.assetBundleName.trim().length > 0);

    if (!targetVocal?.assetBundleName) {
      throw error(404, "No vocal track available for download.");
    }
    ...
    const audioBytes = await fetchBinaryWithAcceleration({
      ...
      onProgress: (loadedBytes, totalBytes) => {
        ...
        progress.fetching(2 + ratio * 78, "fetching-audio");
      }
    });

    progress.fetching(82, "fetching-cover");
    ...
    progress.tagging(88, "writing-metadata");
    const taggedAudio = await tagLib
      .edit(audioBytes, async (file) => {
        const tag = file.tag();
        tag.setTitle(music.title);

        if (music.composer) {
          tag.setArtist(music.composer);
        }

        if (music.arranger) {
          file.setProperty("albumArtist", music.arranger);
        } else if (music.composer) {
          file.setProperty("albumArtist", music.composer);
        }

        tag.setAlbum(music.title);

        if (music.lyricist) {
          file.setProperty("lyrics", music.lyricist);
        }

        if (targetVocal.vocalType) {
          tag.setComment(targetVocal.vocalType);
        }
      ...
      .catch((routeError) => {
        console.error("Failed to embed music metadata on the server.", routeError);
        return audioBytes;
      });
    ...
    const vocalSuffix = targetVocal.vocalType
