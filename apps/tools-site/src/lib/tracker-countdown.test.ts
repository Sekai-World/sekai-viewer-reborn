...
describe("getTrackerCountdown", () => {
  ...
  it("counts down to the event start before a current event begins", () => {
    expect(
      getTrackerCountdown({ startAt, aggregateAt, closedAt: null, now: "2026-01-01T23:59:58.000Z" })
    ).toEqual({
      mode: "starts",
  ...
  it("does not render expired or invalid countdowns", () => {
    expect(
      getTrackerCountdown({ startAt, aggregateAt, closedAt: null, now: aggregateAt })
    ).toBeNull();
    expect(
      getTrackerCountdown({ startAt: "invalid", aggregateAt, closedAt: null, now: startAt })
    ).toBeNull();
  });
  ...
  it("supports Unix seconds and missing starts", () => {
    expect(
      getTrackerChapterCountdown({
        currentStartAt: 1_767_244_800,
        nextStartAt: null,
        currentEndAt: 1_767_291_723_000,
        now: 1_767_248_400_000
      })
    ).not.toBeNull();
    expect(
      getTrackerChapterCountdown({
