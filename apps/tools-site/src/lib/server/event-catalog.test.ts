...
describe("getEventCatalog", () => {
  ...
  it.each(["current", "list"])(
    "returns a terminal network error when the %s request stalls",
    async (stalledRequest) => {
      const pending = new Promise<never>(() => undefined);
      mocks.getEventsByRegionCurrent.mockImplementation(() =>
        stalledRequest === "current"
          ? pending
          : Promise.resolve({ data: { id: 1, name: "Current" } })
      );
  ...
  it.each([
    ...
    mocks.getEventsByRegionCurrent.mockResolvedValue({
      data: { id: "42", name: `Event ${region}`, startAt: "2026-08-01T00:00:00Z", aggregateAt }
    });
    mocks.getEventsByRegionList.mockResolvedValue({
      data: {
        items: [{ id: "42", name: `Event ${region}`, startAt: "2026-08-01T00:00:00Z", aggregateAt }]
      }
    });

    await expect(
      getEventCatalog("https://master.example.test", region as "jp" | "en")
  ...
  it("loads current metadata alongside an explicit selected event", async () => {
    mocks.getEventsByRegionCurrent.mockResolvedValue({ data: { id: 42, name: "Current" } });
    mocks.getEventsByRegionList.mockResolvedValue({
      data: { items: [{ id: 42, name: "Current", startAt: "2026-08-01T00:00:00Z" }] }
    });
