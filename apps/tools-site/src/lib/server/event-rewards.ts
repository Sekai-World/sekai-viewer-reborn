...
export const getEventRewards = async (
  baseUrl: string,
  region: TrackerRegion,
  eventId: number
): Promise<EventRewardsResult> => {
  try {
    const response = await withRequestTimeout<
      Awaited<ReturnType<typeof getEventsByRegionByIdRewards>>
    >((signal) =>
      getEventsByRegionByIdRewards({
        baseUrl,
        path: { region, id: String(eventId) },
        signal
      } as Parameters<typeof getEventsByRegionByIdRewards>[0])
    );
    if ("error" in response && response.error) return { status: "sdk-error", items: [] };
    const items = response.data?.items;
    return Array.isArray(items)
      ? { status: "available", items }
      : { status: "invalid-data", items: [] };
  } catch {
