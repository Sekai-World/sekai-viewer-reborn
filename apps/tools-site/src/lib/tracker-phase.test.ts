...
import {
  getNextTrackerRefreshCountdownMs,
...
describe("tracker phase", () => {
  ...
  it("classifies upcoming, live, and finished boundaries", () => {
    expect(getTrackerPhase({ startAt, aggregateAt, now: "2025-12-31T23:59:59.999Z" })).toBe(
      "upcoming"
    );
  ...
  it("uses unavailable for invalid or inverted dates", () => {
    ...
    expect(getTrackerPhase({ startAt: aggregateAt, aggregateAt: startAt, now: startAt })).toBe(
      "unavailable"
    );
  });
  ...
  it("schedules exactly two terminal refreshes after aggregation", () => {
    ...
    expect(
      getNextTrackerRefreshDeadline({ now: terminalAggregateAt, aggregateAt: terminalAggregateAt })
    ).toBe(firstTerminal);
    expect(
      getNextTrackerRefreshDeadline({
        now: "2026-01-01T13:10:10.000Z",
        aggregateAt: terminalAggregateAt
      })
    ).toBe(secondTerminal);
    expect(
      getNextTrackerRefreshDeadline({
        now: "2026-01-01T13:12:00.000Z",
        aggregateAt: terminalAggregateAt
      })
    ).toBe(secondTerminal);
    expect(
      getNextTrackerRefreshDeadline({
        now: "2026-01-01T13:15:10.000Z",
        aggregateAt: terminalAggregateAt
      })
    ).toBeNull();
  });
  ...
  it("does not schedule invalid refresh schedules", () => {
    ...
    expect(
      getNextTrackerRefreshDeadline({ now: startAt, aggregateAt, offsetMs: Number.NaN })
    ).toBeNull();
  });
});
