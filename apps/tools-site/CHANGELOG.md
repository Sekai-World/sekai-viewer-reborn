...
- cc5d79c: Hide the tracker data export controls while the export format is being reconsidered.

### Patch Changes

- 7382b8f: Keep the shared audio-player progress display synchronized after seeking during playback.
- fc085da: Keep World Bloom chapter metadata when a chapter has no character ID, but do not load or show a chapter ranking for it.
...
- b70a3c7: Restore the event tracker's ranking-card hover and focus tint and the current-chapter tab highlight by resolving their backgrounds from the shared panel surface token.
- 1e12256: Add event filtering, CSV/XLSX export, shareable tracker links, and a constant-pace goal calculator to the event tracker. Tune the goal dialog for dark mode with a restrained surface and a reduced-transparency-safe blurred backdrop.
