...
export default async function action_motion(controller: Live2DController, action: Snippet) {
  ...
  if (action_detail.Type === CharacterLayoutType.CharacterMotion) {
    log.log("Live2DController", "CharacterMotion/CharacterMotion", action, action_detail);
