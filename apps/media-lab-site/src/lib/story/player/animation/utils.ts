 ...
 */
export function texture_slice(base_texture: BaseTexture, parts: [number, number], total?: number) {
  ...
  for (let h = 0; h < parts[1]; h++) {
    for (let w = 0; w < parts[0]; w++) {
      ret.push(new Texture(base_texture, new Rectangle(w * width, h * height, width, height)));
      count++;
