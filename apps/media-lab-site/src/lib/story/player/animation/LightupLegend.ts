...
export default class LightupLegend extends BaseAnimation {
  constructor(textures: ILive2DTexture[], fog_type: string) {
    ...
    const base_texture = this.textures.find((a) => a.identifier === "ui/tex_light_up_legend")!
      .texture.baseTexture;
    const fog_t = new Texture(base_texture, new Rectangle(0, 0, 256, 256));
    const camera_light_t = new Texture(base_texture, new Rectangle(256, 0, 256, 256));
    const camera_light2_t = new Texture(base_texture, new Rectangle(0, 256, 256, 256));
    ...
    if (fog_type === "corner") {
      fog_l.forEach((s) => {
        const factor = 0.5;
        s.position[0] =
          s.position[0] < 0.5 ? s.position[0] * factor : 1 - (1 - s.position[0]) * factor;
        s.position_to[0] =
          s.position_to[0] < 0.5 ? s.position_to[0] * factor : 1 - (1 - s.position_to[0]) * factor;
      });
    }
    fog_l.forEach((s) => {
