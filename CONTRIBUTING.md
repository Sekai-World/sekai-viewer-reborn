...
Visual regression is optional and is not an automatic CI gate. The
`pnpm test:visual` command remains available for rendered UI changes. Approved
baselines target Ubuntu Chromium, so generate candidate snapshots with
`pnpm test:visual:update` only in that environment. On other systems, manually
dispatch **Visual baseline candidate** from the repository's default branch with
the open PR number and `update_snapshots` explicitly enabled; this is the
supported Linux baseline path. The workflow resolves that PR's head SHA through
the read-only GitHub API, checks out that exact SHA, and uploads an artifact
named with the PR number and SHA; it has no permission to push or commit.
Download and review the candidate PNGs, then copy approved snapshots into the
branch and commit them as part of the pull request. Do not directly overwrite
baselines from CI.
